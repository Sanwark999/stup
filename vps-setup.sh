```bash
cat > /root/vps-setup.sh << 'SCRIPTEOF'
#!/bin/bash
set -e
...
[Peer]
# Home PC (WSL) - AllowedIPs=0.0.0.0/0 so WireGuard routes ALL traffic to home
PublicKey    = $HOME_PUB
...
sleep 2
systemctl is-active --quiet wg-quick@wg0 && ok "WireGuard running!" || fail "WireGuard failed"
# Policy routing: phone traffic goes through home PC
